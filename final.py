...
conn = sqlite3.connect('ecsel_database.db')
df_chart = pd.read_sql_query("""SELECT country, activityType, SUM(ecContribution) FROM participants WHERE country = '{}' """.format(selectedacronym), conn)

conn.close()
df_chart = df_chart.rename(columns=columnnamechanges)
print(df_chart)
st.dataframe(df_chart) 
st.line_chart(data=dfchart, use_container_width=True, width=0, height=0, x_axis_label='activityType', y_axis_label='Contribution Sum')
